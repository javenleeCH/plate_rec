...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        bbox_thick = int(0.6 * (image_h + image_w) / 600)        # box size /600
        ...
        if show_label:
            ...
            t_size = cv2.getTextSize(
                bbox_mess, 0, fontScale, thickness=bbox_thick//2)[0]          # text size //2
            cv2.rectangle(
                image, c1, (c1[0] + t_size[0], c1[1] - t_size[1] - 3), bbox_color, -1)  # filled
            if is_chinese(classes[class_ind]):
                cv2img = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
                pilimg= Image.fromarray(cv2img)
                draw = ImageDraw.Draw(pilimg)
                font = ImageFont.truetype("simhei.ttf", 15, encoding="utf-8")
                draw.text((c1[0], c1[1]-13), bbox_mess, font=font,fill='black')
                image = cv2.cvtColor(np.array(pilimg), cv2.COLOR_RGB2BGR)
            else:
                cv2.putText(image, bbox_mess, (c1[0], c1[1]-2), cv2.FONT_HERSHEY_SIMPLEX,
                            fontScale, (0, 0, 0), bbox_thick//2, lineType=cv2.LINE_AA)           # text size //2
